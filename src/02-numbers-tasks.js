...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const modVectA = Math.sqrt(x1 ** 2 + y1 ** 2);
  const modVectB = Math.sqrt(x2 ** 2 + y2 ** 2);
  const cosAB = scalProd / (modVectA * modVectB);
...
function getParallelipidedDiagonal(a, b, c) {
  const sqrtNumbers = a ** 2 + b ** 2 + c ** 2;
  return Math.sqrt(sqrtNumbers);
}
